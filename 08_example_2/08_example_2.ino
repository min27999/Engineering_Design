...
void loop() {
  ...
  if (distance < _DIST_MIN) {
    distance = lastDistance;    // Set Lower Value
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (distance > _DIST_MAX) {
    distance = lastDistance;    // Set Higher Value
    digitalWrite(PIN_LED, 1);       // LED OFF
...
void ledControl(float dist)
{
  if (dist <= 200)
    analogWrite(PIN_LED, 255 - dist * 255 / 100);
  else
    analogWrite(PIN_LED, dist * 255 / 100);
}
